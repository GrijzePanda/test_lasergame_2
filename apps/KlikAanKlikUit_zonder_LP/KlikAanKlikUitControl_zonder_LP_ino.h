...
#include "crt_VeerKnop.h"
namespace crt
{
 	MainInits mainInits;            // Initialize CleanRTOS

	const int gpioPinVeerKnop = 34; // GPIO pin number for the VeerKnop
	const int gpioPinLed = 22;      // GPIO pin number for the LED

	KlikAanKlikUitControl klikAanKlikUitControl("KlikAanKlikUitControl", 2 /*priority*/, 4000 /*stackBytes*/, ARDUINO_RUNNING_CORE, gpioPinLed); // Don't forget to call its start() memeber during setup().
	VeerKnop     		  veerKnop  ("VeerKnop",   2 /*priority*/, 4000 /*stackBytes*/, ARDUINO_RUNNING_CORE, klikAanKlikUitControl, gpioPinVeerKnop);
}
